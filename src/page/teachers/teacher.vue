<template>  
  <div class="teacher-profile">  
    ...
    <el-input style="width: 310px" placeholder="输入新密码" v-model="passwordForm.password" show-password required @input="validatePassword" >
    <template #prepend>
      <el-icon>
        <Lock />
      </el-icon>
...
<el-form-item label="确认新密码:" class="password-input">
  <el-input  style="width: 310px" placeholder="确认新密码" v-model="passwordForm.password2" show-password required @input="checkPasswordMatch" >
    <template #prepend>
      <el-icon>
        <Lock />
      </el-icon>
    ...
    <template #suffix>
      <el-icon :name="passwordMatch ? 'el-icon-circle-check' : 'el-icon-circle-close'" :color="passwordMatch ? 'green' : 'red'"></el-icon>
    </template>
  </el-input>
...
.password-message {
  text-align: right; /* 保证文本对齐到右侧 */
  margin-top: 5px; /* 适当的顶部间距 */
}

.text-success {
  color: green; /* 密码匹配时的文本颜色 */
}

.text-danger {
  color: red; /* 密码不匹配时的文本颜色 */
}
.password-input {
    display: flex;
    align-items: center;
  ...
  .password-message {
    margin-left: 10px; /* 调整输入框和提示信息之间的间距 */
  }
</style>